...
const createTask = async (pool, task) => {
    const { questname, description, position, reward, selectedOption, endTime, timestamp } = task;
    ...
    try {
      const result = await client.query(
        ...
        [questname, description, position, reward, selectedOption, '1', endTime, timestamp]
      );
